...
bool computepath(int xFinish, int yFinish, int xStart, int yStart)
{
	...
	while (openlist.empty() == false)         //A*search
	{
		...
		if (openlist.empty() == false)
		{
			...
			{
				if (current->gvalue >= openlist.top().gvalue)
				{
					x = current->xPos;
					y = current->yPos;
				}
...
void set_map()      //function to generate the map
{
	ifstream file;
...
int main()
{
	...
	while (1)
	{
		...
		finish = clock();
		duration = (double)(finish - start) / CLOCKS_PER_SEC;   //calculate run time
