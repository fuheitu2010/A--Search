...
public:
	...
	void calculate_fvalue(int gx,int gy)
	{
		...
		xd = gx - xPos;
		yd = gy - yPos;
		hvalue = abs(xd) + abs(yd);       //calculate hvalue
...
bool computepath(int xStart,int yStart,int xFinish,int yFinish)
{
	 priority_queue<node> openlist; // list of open nodes
	 node* current;
	 node* sub;
	 int i, x, y, xdx, ydy;
	...
	while (openlist.empty()==false)         //A*search
	{
		...
		if (openlist.empty() == false)
		{
			...
			{
				if (current->gvalue >= openlist.top().gvalue)   //compare gvalue
				{
					x = current->xPos;
					y = current->yPos;
				}
		...
		for (i = 0; i<4; i++)             //Generate subnoe of current node
		{
			...
			if ((xdx>0) & (xdx<n - 1) & (ydy>0) & (ydy<m - 1) & (close[xdx][ydy] != 1)&(map[xdx][ydy]!=1))
			{
...
void set_map()                 //function to set the map
{
	ifstream file;
...
int main()
{
	...
	finish = clock();
	duration = (double)(finish - start) / CLOCKS_PER_SEC;   //calcuolate the run time
	
	cout << "The compute time is:" << duration << "seconds" << endl;
	if (find == false)
	{
	...
	else
	{
		...
		for (int i = 0; i < 101; i++)
		{
			...
			{
				if (map[i][j] == 2)
					cout << "S";                   //indicate start point
				else if (map[i][j] == 3)
					cout << "*";                     //mark the route       
				else if (map[i][j] == 4)
					cout << "G";              //idicate finish point
				else
					cout << map[i][j];
			}
				cout << endl;
